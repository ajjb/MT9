...
public class SpiderLeg {
	...
	public boolean crawl(String url) {
		try {
			...
			{
				System.out
						.println("\n**Visitando ** Recebido pagina web em " + url);
			}
			if (!connection.response().contentType().contains("text/html")) {
				System.out
						.println("**Falha** tentando em outra pagina HTML");
				return false;
			}
			Elements linksOnPage = htmlDocument.select("a[href]");
			System.out.println(" Encontrado (" + linksOnPage.size() + ") links");
	...
	public boolean searchForWord(String searchWord) {
		...
		if (this.htmlDocument == null) {
			System.out
					.println("ERROR! chamando crawl() trabalhando na analise do documento");
			return false;
		}
		System.out.println("pesquisando pela palavra " + searchWord + "...");
